...
def test_sanity():
    ...
    assert isinstance(sensor_firmware_version, float), "Sensor firmware version is not a float"

    sensor_reading_interval = sensor_info.get("reading_interval")
    assert isinstance(sensor_reading_interval, int), "Sensor reading interval is not an integer"

    sensor_reading = get_sensor_reading()
    assert isinstance(sensor_reading, float), "Sensor doesn't were to register temperature"
