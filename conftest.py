...
class SensorMethod(Enum):
    ...
    UPDATE_FIRMARE = "update_firmware"
...
def wait(func: Callable, condition: Callable, tries: int, timeout: int, **kwargs):
    for i in range(tries):
        try:
            ...
            log.debug(f"Evaluating result of the call with function {condition.__name__}")
            if condition(result):
                return result
...
@pytest.fixture(scope="session")
def sensor_update_firmware(make_valid_request):
    def _sensor_update_firmware():
        log.info("Send firmware update request to sensor")
        return make_valid_request(SensorMethod.UPDATE_FIRMARE)

    return _sensor_update_firmware
